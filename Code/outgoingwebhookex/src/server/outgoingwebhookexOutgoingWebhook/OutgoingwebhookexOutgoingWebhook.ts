...
/**
 * Implementation for outgoingwebhookex Outgoing Webhook
 */
@OutgoingWebhookDeclaration("/api/webhook")
export class OutgoingwebhookexOutgoingWebhook implements IOutgoingWebhook {
    ...
    public requestHandler(req: express.Request, res: express.Response, next: express.NextFunction) {
        ...
        if (securityToken && securityToken.length > 0) {
            ...
            if (msgHash === auth) {
                ...
                const scrubbedText = OutgoingwebhookexOutgoingWebhook.scrubMessage(incoming.text);
                message = OutgoingwebhookexOutgoingWebhook.processAuthenticatedRequest(scrubbedText);
            } else {
    ...
    private static processAuthenticatedRequest(incomingText: string): Partial<builder.Activity> {
        ...
        if (!selectedPlanet || !selectedPlanet.length) {
            ...
            const adaptiveCard = OutgoingwebhookexOutgoingWebhook.getPlanetDetailCard(selectedPlanet[0]);
            message.type = "result";
